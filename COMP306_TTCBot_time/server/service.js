...
service.get('/service/:location', (req, res, next) => {
        ...
        const station = req.params.location;
        const timestamp = +moment().format('X');

        request.get('https://myttc.ca/finch' + location + '_station.json', (err, response) => {
            if (err) {
                console.log(err);
            ...
            const next = result.stops[1].routes[-3].stop_times[0].departure_time;
